...
const CoursesDashboard: React.FC = () => {
  ...
  const fetchCoursesData = async () => {
    try {
      ...
      // Grant trial access if user doesn't have any access yet
      if (!accessData || accessData.length === 0) {
        await grantTrialAccess();
      }
    } catch (error) {
  ...
  const grantTrialAccess = async () => {
    try {
      // Grant trial access to all courses
      const trialAccessData = courses.map(course => ({
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center">
                    ...
                    <div className="text-sm text-gray-400">
                      Chapter {chapter.order_position}
                    </div>
                  </div>
          ...
          <div className="bg-dark-light rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {courses.map((course) => {
                ...
                const discountedPrice = course.price - (course.price * course.discount_percentage / 100);
