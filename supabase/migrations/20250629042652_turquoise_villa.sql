/*
  ...
  1. New Tables and Functions
    - Create referral code generation functions
    - Update user_profiles table with referral fields
    - Create discount_codes table for coupon management
    - Create course_purchases table for tracking referral commissions
    - Update user_courses table for enrollment tracking

  2. Security
    - Enable RLS on all tables
    - Add policies for user data access
    - Ensure proper referral tracking
...
-- Update course_purchases table if it exists, create if it doesn't
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'course_purchases') THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'course_purchases' AND column_name = 'referrer_id'
    ) THEN
      ALTER TABLE course_purchases ADD COLUMN referrer_id UUID REFERENCES user_profiles(user_id);
    END IF;

    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'course_purchases' AND column_name = 'discount_code'
    ) THEN
      ALTER TABLE course_purchases ADD COLUMN discount_code TEXT REFERENCES discount_codes(code);
    END IF;
  ELSE
...
-- Update user_courses table if it exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_courses') THEN
    -- Add referral_code_used column if it doesn't exist
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'user_courses' AND column_name = 'referral_code_used'
    ) THEN
      ALTER TABLE user_courses ADD COLUMN referral_code_used VARCHAR(10);
    END IF;
  END IF;
END $$;
...
-- RLS Policies for user_profiles (only add if they don't exist)
DO $$
BEGIN
...
-- RLS Policies for user_courses (only if table exists and policies don't exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_courses') THEN
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE tablename = 'user_courses' AND policyname = 'Users can view own courses'
    ) THEN
...
-- Insert sample discount codes for existing users (only if they don't already have them)
