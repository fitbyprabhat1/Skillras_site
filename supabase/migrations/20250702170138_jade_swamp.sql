/*
  ...
  1. New Tables
    - `coupon_codes` - Store discount coupon codes with validation
    - `payment_links` - Different payment URLs for discount tiers
    - `user_kyc` - KYC information for Indian compliance
    - `course_enrollments` - Track course enrollments and payments
    - `payment_transactions` - Payment transaction records

  2. Security
    - Enable RLS on all tables
    - Add policies for user data access and management
    - Secure coupon code validation

  3. Functions
    - Coupon code validation and application
    - Payment link routing based on discount tiers
*/
...
-- Course enrollments table (using course_id as text to avoid foreign key issues)
CREATE TABLE IF NOT EXISTS course_enrollments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  course_id text NOT NULL, -- Store course ID as text instead of foreign key
  course_name text NOT NULL, -- Store course name for reference
